...
- After 0.6.5 (2026-09-26, pending S11 check): **🎲 pick visible** — each generated image remembers the drawn character (upscales keep it). The 캐릭터 tab shows a banner with the current image's pick (name large, series below; a Danbooru "(series)" suffix is not repeated) and "이 캐릭터로 고정" (writes it into the first character, turns 🎲 off); before the first draw it says the pick will appear there. A toast "🎲 name" appears after each random generation; the viewer's meta line starts with "🎲 name · series"; session thumbnails name it for accessibility only (nothing drawn over images). Re-roll = 생성 again. **Usage hint:** with one percent missing the time is said once ("가득 차기까지 약 …", or "최대 약 …" when only the rate is known), and the implied daily rate follows ("하루 약 11%" for 7888 s; omitted above 100 %/day).
- 0.6.7 (2026-09-26): the user found NovelAI refills about 190 images a day; at the observed 7888 s per percent (≈ 10.95 %/day) one percent ≈ 17 standard images. The usage hint starts with "약 N장 분량" and the daily rate reads "하루 약 11%(190장)". Estimate only: cost varies with size/steps.
